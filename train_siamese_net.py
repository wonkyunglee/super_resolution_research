...
def train_single_epoch(config, model, dataloader, criterion,
    ...
    for i, (LR_patch, HR_patch, filepath) in tbar:
        if i % 3 == 0:
            ...
            HR_patch_last = HR_patch
            LR_patch_last = LR_patch
            label = 1
            print(1)
        elif i % 3 == 1:
            HR_patch = HR_patch.to(device)
            LR_patch = LR_patch_last
            label = 0
            print(2)
        elif i % 3 == 2:
            HR_patch = HR_patch_last
            LR_patch = LR_patch.to(device)
            label = 0
            print(3)

        pred_dict_lr = model.forward(LR=LR_patch)
        pred_dict_hr = model.forward(HR=HR_patch)
...
def evaluate_single_epoch(config, model, dataloader, criterion,
    ...
    with torch.no_grad():
        ...
        for i, (LR_img, HR_img, filepath) in tbar:
            ...
            loss += criterion['val'](pred_dict_hr=pred_dict_hr,
                                     pred_dict_lr=pred_dict_lr,
                                     label=label)['loss']
