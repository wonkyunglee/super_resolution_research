...
def train_single_epoch(config, student_model, teacher_model, dataloader, criterion,
    ...
    for i, (LR_patch, HR_patch, filepath) in tbar:
        ...
        student_pred_dict = student_model.forward(LR=LR_patch, HR=HR_patch)
        loss = criterion['train'](student_pred_dict, HR_patch)
...
def parse_args():
    parser = argparse.ArgumentParser(description='noisy student network')
