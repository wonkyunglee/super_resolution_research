...
def float2uint8(image, normalize=False):
...
def step2_visualizer(rgb_range):
    def get_figure(LR, HR, pred_student, pred_teacher):
        ...
        fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 4, figsize=(16,4))
        cmap = 'gray'
        ax1.imshow(float2uint8(quantize(pred_student_residual_hr, rgb_range)), cmap=cmap)
        ax1.set_title('pred_s_residual_hr, mean_val : %.4f'% pred_student_residual_hr.mean())
        ax2.imshow(float2uint8(quantize(pred_student_hr, rgb_range)), cmap=cmap)
        ax2.set_title('pred_s_hr, mean_val : %.4f'%pred_student_hr.mean())
        ax3.imshow(float2uint8(quantize(residual_diff, rgb_range)), cmap=cmap)
        ax3.set_title('residual_diff, mean_val : %.4f'%residual_diff.mean())
        ax4.imshow(float2uint8(quantize(HR - pred_student_hr, rgb_range)), cmap=cmap)
...
def step2_attention_visualizer(rgb_range):
    def get_figure(LR, HR, pred_student, pred_teacher):
        ...
        ax4.imshow(float2uint8(quantize(HR - pred_student_hr, rgb_range)), cmap=cmap)
        ax4.set_title('GT - pred_hr, mean_val : %.4f'%(HR-pred_student_hr).mean())
        ax5.imshow(float2uint8(quantize(attention, rgb_range)), cmap=cmap)
