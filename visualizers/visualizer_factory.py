...
def get_figure_basic(LR, HR, pred):
    ...
    fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 3, figsize=(16,4))
    ax1.imshow(float2uint8(pred_residual_hr))
    ...
    ax3.imshow(float2uint8(HR))
    ax3.set_title('ground_truth, mean_val : %.4f'%HR.mean())
    ax4.imshow(float2unit8(HR - pred_hr))
...
def step2_visualizer():
    def get_figure(LR, HR, pred_student, pred_teacher):
        ...
        residual_diff = pred_teacher_residual_hr - pred_student_residaul_hr
        fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 3, figsize=(16,4))
        ax1.imshow(float2uint8(pred_studnet_residual_hr))
        ax1.set_title('pred_s_residual_hr, mean_val : %.4f'% pred_studnet_residual_hr.mean())
        ax2.imshow(float2uint8(pred_student_hr))
        ax2.set_title('pred_s_hr, mean_val : %.4f'%pred_student_hr.mean())
        ax3.imshow(float2uint8(residual_diff))
        ax3.set_title('residual_diff, mean_val : %.4f'%residual_diff.mean())
        ax4.imshow(float2unit8(HR - pred_student_hr))
        ax4.set_title('GT - pred_hr, mean_val : %.4f'%(HR-pred_student_hr).mean())
    
    return get_figure


def get_visualizer(config, optimizer, last_epoch):
